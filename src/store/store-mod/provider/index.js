...
export default{
  ...
  actions:{
    // 'actions -> mutations' 으로 값을 전달할 때는 'commit' 사용
    ...
    fnRegisterUser({commit}, payload){
      ...
      .then(async (pUserInfo) => {
        ...
        const user = {
          ...
          name: payload.pName,
          phoneNumber: payload.pPhoneNumber,
          address: payload.pAddress,
          detailAddress: payload.pDetailAddress,
          termsAgreement: payload.pTerms,
          personalAgreement: payload.pPersonal,
          freeServiceAgreement: payload.pFreeService
        };

        await firebase.firestore().collection("users").doc(pUserInfo.user.uid).set(user);

        // 신규 회원 이메일 정보를 스토어에 저장
        commit('fnSetUser', user);
        commit('fnSetLoading', false) // 스토어에 시간완료 상태 변경
        commit('fnSetErrorMessage', '') // 스토어 에러메시지 초기화
        router.push('/joinFinish') // 로그인 후 회원가입 완료 화면으로 이동
      })
        .catch(err => {
    ...
    fnDoLogin({commit}, payload){
      ...
      firebase.auth().signInWithEmailAndPassword(payload.pEmail, payload.pPassword).then(pUserInfo => {
